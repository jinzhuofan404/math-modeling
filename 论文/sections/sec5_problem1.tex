...
\begin{itemize}
    \item 一致性指数（C-index）：\textbf{0.8986}（修复信息泄漏后，模型仍保持了良好的判别能力）
    \item 预测平均绝对误差（MAE）：\textbf{1.33天}（较前版的5.03天大幅改善，因修正流失定义后多数玩家在数天内快速流失）
\end{itemize}
...
\begin{itemize}
    ...
    \end{itemize}
    与前版（使用全周期特征，C-index=0.9479）相比，修复信息泄漏后D3模型的系数结构发生了显著变化。前版的``最后狂欢''效应（日均活跃度$\beta=+0.57$）在纯前3天特征中不再出现，支持了该效应是信息泄漏伪影的假说。
\end{itemize}
