...
\toprule
\textbf{等级段} & \textbf{钻石比} & \textbf{矿石消耗比} & \textbf{金币持有率} & \textbf{金币均值（有金币者）} & \textbf{主因} \\
\midrule
1--3级 & 停滞:非停滞 = 1:2 & 均为0 & $<$1\% & 均为0 & 钻石稀缺 \\
4--6级 & 持平(600 vs 594) & 停滞者8.2$\times$ & 57\% vs 39\% & 50万 vs 14万 & 资源效率递减 \\
\bottomrule
\end{tabular}
\end{table}

注：7级及以上玩家仅占训练集8\%（158/2000），且该阶段停滞玩家仅1人，无法进行有统计意义的对比分析——大多数玩家在到达7级之前已充分成长或已流失。因此瓶颈分析聚焦1--6级。

\textbf{早期瓶颈（1--3级）：钻石稀缺。} 停滞玩家的钻石存量仅为非停滞者的50\%（100 vs 200），其余资源消耗模式几乎相同。金币在该阶段尚未激活（$<$1\%玩家持有）——金币经济从3--4级才逐渐解锁，与玩家进入中期阶段同步。早期瓶颈的关键词：\emph{缺钻卡级}。

\textbf{中期瓶颈（4--6级）：资源效率递减。} 停滞玩家消耗了8.2倍矿石、4.3倍粮食/木材，活跃天数为2倍（10天 vs 5天），但日均升级速度仍低于0.1级/天。与此同时，停滞玩家的金币持有率（57\%）和持有量（均值50万）均显著高于非停滞玩家（39\%，均值14万）——金币不是瓶颈的解决者，而是\emph{停滞后无处可花的堆积物}。此阶段的关键词：\emph{刷不动了，金币花不出去}。
...
\textbf{金币与钻石扮演截然不同的角色。} 钻石是\emph{瓶颈打破者}——存量越高，停滞概率越低；金币是\emph{停滞堆积物}——玩家卡级期间金币无法有效转化为升级进度，反而大量囤积。在4--6级停滞玩家中，金币持有率高达57\%（vs 非停滞的39\%），持币者均值达50万（vs 非停滞的14万），但升级速度仍显著落后。在等级4+全体玩家的偏相关分析中，金币与日均增长的关联度（$r=-0.143$）在五类资源中最低，远弱于矿石（$r=-0.462$）和粮食/木材（$r\approx-0.427$），进一步确认了金币\"堆积而不用\"的特征。
