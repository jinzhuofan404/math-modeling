...
\textbf{数据窗口说明：} 问题2的研究目标是资源消耗、成长速度与付费行为的关联分析，题目未限制时间窗口。本节使用全周期数据进行分析。\textbf{付费金额说明：} 经查验原始数据中的支付事件记录，付费相关字段（total\_pay）以美元标价（0.99 USD对应标准6元首充档），本文统一按1 USD $\approx$ 6 CNY折算为人民币呈现。与问题1的预测任务（仅前3天）和问题3的策略模拟（综合前两问结果）形成互补。
...
对75名付费用户（占总体的3.8\%）进行分析。注：训练数据中付费金额字段以美元标价，本文统一按1 USD $\approx$ 6 CNY折算为人民币。

\begin{itemize}
    \item 首付等级：100\%的付费用户首次付费发生在1级（注册初期）
    \item 付费金额分布：中位数约6元（即0.99美元首充档），75\%分位数约12元
    \item 付费金额与留存的关系：高付费组（600元+）平均生命周期71.0天，低付费组（$<$36元）平均26.8天\\
